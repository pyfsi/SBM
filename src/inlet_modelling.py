...
class InletModel():
    ...
    def define_bubble(self, C_ID, C_t, t, mg_bubble):
    ...
    def insert_bubbles(self, t_insert_idx) -> float:
        ...
        while abs(mg_per_insert-mg_inserted) > (tol_mg):
            ...
            is_bubble_defined, mg_defined = self.define_bubble(C_ID, C_t, t_insert_idx, mg_bubble)
...
def inlet_modelling(config):
    ...
    for t_insert_idx in range(n_time_insert - 1): # no insertion at last time step
        start_msg = f"Start bubble calculation for time interval {t_insert_idx}"
